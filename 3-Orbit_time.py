...
def orbit(T,M,m,e,theta,phi,t):
    ...
    ang=2*np.pi
    if t<=T/2:
        sol = np.linspace(0, np.pi, 200)
        for i in sol:
            dif=np.abs(t-T*(2*np.arctan(sqrt((1-e)/(1+e))*tan(i/2))-e*sqrt(1-e**2)*sin(i)/(1+e*cos(i)))/(2*np.pi))
            if dif<difmin:
                difmin=dif
                ang=i
    else:
        sol = np.linspace(np.pi, 2*np.pi, 200)
        for i in sol:
            dif = np.abs(t -T- T * (2 * np.arctan(sqrt((1 - e) / (1 + e)) * tan(i / 2)) - e * sqrt(1 - e ** 2) * sin(i) / (1 + e * cos(i))) / (2 * np.pi))
            if dif < difmin:
                difmin = dif
                ang = i
...
orb1=orbit(365,1.9E15,3E14,0.7,0,0,6)
orb2=orbit(365,1.9E15,3E14,0.7,0,0,17)
orb3=orbit(365,1.9E15,3E14,0.7,0,0,51)
orb4=orbit(365,1.9E15,3E14,0.7,0,0,182)
orb5=orbit(365,1.9E15,3E14,0.7,0,0,313)
orb6=orbit(365,1.9E15,3E14,0.7,0,0,347)
orb7=orbit(365,1.9E15,3E14,0.7,0,0,358)
orb8=orbit(365,1.9E15,3E14,0.7,0,0,365)
